...
def run(trainp="../data/WebQSP.train.json", testp="../data/WebQSP.test.json"):
    ...
    print_forms(tq2p, trainquestions, traingraphs, tofile="../data/WebQSP.train.lin")
    print_forms(xq2p, testquestions, testgraphs, tofile="../data/WebQSP.test.lin")


def print_forms(tq2p, trainquestions, traingraphs, tofile=None):
                ...
                for entitynameset in entnames:
                    ...
                    for entitynamesetel in entitynameset:
                        if entitynamesetel in language_form:
                            ...
                            break
                        else:   # try to find substring match
    ...
    print "{} out of {} questions have non-matching entity labels -> {}%".format(len(missing_entity_names), total, (total - len(missing_entity_names)) / (1.*total))
            #print entity_names
    #embed()
...
def find_partial(key, hay):
    if "patriots" in hay:
        pass
    haywords = tokenize(hay); keywords = tokenize(key)
...
def buildgraphs(d, no_value_constraints=False, no_entity_constraints=False, no_order=False, no_complex_order=True):
    ...
    print "number of questions with:\n\tentity constraints: {} ({})\n\tvalue constraints: {}\n\torder: {} ({} of which argmax, {} others)"\
        .format(withentconstraintcount, onlyentconstraintcount, withvalconstraintcount, withordercount, argoptcount, otherordercount)
